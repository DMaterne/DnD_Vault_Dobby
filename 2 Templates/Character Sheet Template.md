...
const c = [dv.page](http://dv.page/)(CHARACTER_PATH);  
...
  return [dv.page](http://dv.page/)(resolvedPath);  
}  
...
  if (key === "int") return Number([c.int](http://c.int/) ?? 10);  
...
    .filter(p => p)  
    .sort((a, b) => {  
      const nameA = String([a.name](http://a.name/) ?? a.file?.name ?? "");  
      const nameB = String([b.name](http://b.name/) ?? b.file?.name ?? "");  
...
    const itemPage = [dv.page](http://dv.page/)(itemPath);  
...
  if (key === "int") return applyItemEffects(Number([c.int](http://c.int/) ?? 10), "int");  
...
  return applyItemEffects(Number([c.ac](http://c.ac/) ?? 10), "ac");  
}  
...
  const itemPage = [dv.page](http://dv.page/)(resolvedItemPath);  
  if (!itemPage) return;  
...
        file: {  
          name: [action.name](http://action.name/) ?? itemPage.name ?? itemPage.file?.name ?? "Unnamed Action",  
...
        source_label: itemPage.name ?? itemPage.file?.name ?? "Unknown Item",  
        file: {  
          name: [action.name](http://action.name/) ?? itemPage.name ?? itemPage.file?.name ?? "Unnamed Spell",  
          path: itemPage.file?.path ?? itemPath  
        }  
      });  
    }  
...
  const title = headerText.createEl("div", { text: [c.name](http://c.name/) ?? "Unbenannter Charakter" });  
...
  portrait.alt = [c.name](http://c.name/) ?? "Character Portrait";  
...
        const itemPath = resolvePathRef(entry?.item, ITEMS_FOLDER);  
        const itemPage = itemPath ? [dv.page](http://dv.page/)(itemPath) : null;  
...
      const leftWrap = summary.createEl("div");  
      leftWrap.createEl("div", { text: [item.name](http://item.name/) });  
...
        .sort((a, b) => a.name.localeCompare([b.name](http://b.name/), "de"));  
...
          const name = String([action.name](http://action.name/) ?? action.file?.name ?? "").toLowerCase();  
...
            text: [action.name](http://action.name/) ?? action.file?.name ?? "Unnamed Action"  
          });  
...
          const fields = [  
            String([spell.name](http://spell.name/) ?? spell.file?.name ?? ""),  
            String(spell.school ?? ""),  
...
          const leftSummary = leftWrap.createEl("div", {  
            text: [spell.name](http://spell.name/) ?? spell.file?.name ?? "Unnamed Spell"  
          });  
          leftSummary.style.fontSize = "1.05em";  
...
      const nameEl = leftWrap.createEl("div", { text: item.displayName ?? [item.name](http://item.name/) });  
