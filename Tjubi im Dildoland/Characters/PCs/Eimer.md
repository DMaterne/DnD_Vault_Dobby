...
```dataviewjs  
const CHARACTER_PATH = "Public/3 Backend/KLLANK-R.md";
...
const portraitPath = c.portrait ?? "Public/1 Assets/Bilder/KLLANK.png";
...
function renderActions() {
  ...
  const actions = dv.pages('"Private/Actions"')
    .where(p => {
...
function renderSpells() {
  ...
  const spellcastingAbility = c.spellcasting_modifier ?? c.spellcasting_ability ?? "int";
  ...
  const spells = dv.pages('"Private/Actions"')
    .where(p => String(p.category ?? "").toLowerCase() === "spell")
    .sort(p => [p.level ?? 99, p.name ?? p.file.name], "asc");
